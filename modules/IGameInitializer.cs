...
namespace framework.modules
{
    public partial class IGameInitializer : Node
    {
        ...
        {
            var eventBus = ServiceLocator.Instance.GetService<IEventBus>("EventBus");
            if (eventBus != null)
            {
                // 订阅全局事件
        ...
        {
            var resourceManager = ServiceLocator.Instance.GetService<IResourceManager>("ResourceManager");
        ...
        private void SaveGameSettings()
        {
            ...
            var settings = new GameSettings
            {
                ...
                MasterVolume = ServiceLocator.Instance.GetService<IAudioManager>("AudioManager")?.MasterVolume ?? 1.0f,
                BgmVolume = ServiceLocator.Instance.GetService<IAudioManager>("AudioManager")?.BgmVolume ?? 1.0f,
                SfxVolume = ServiceLocator.Instance.GetService<IAudioManager>("AudioManager")?.SfxVolume ?? 1.0f,
